...
sb.ViewerSmorg = sb.Smorg.subType({
   ...
   draw:function() {
    ...
    update:function() {
        ...
        if(!this.trackPlayer){  //camera under key control
            var newY = this.position.y;
            var newX = this.position.x;
            if(sb._LISTENERS.left){var newX = this.position.x - 20;}
            if(sb._LISTENERS.right){var newX = this.position.x + 20;}
            if(sb._LISTENERS.up){var newY = this.position.y - 20;}
            if(sb._LISTENERS.down){var newY = this.position.y + 20;}



            this.position.init(newX,newY);
        } else {
